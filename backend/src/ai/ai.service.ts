...
export class AiService {
  ...
  /**
   * Processes conversational chats with the AI planning assistant.
   */
  async chatWithAssistant(userId: string, userMessage: string, history: any[] = []) {
    ...
    try {
      ...
      const response = await axios.post(`${this.geminiUrl}?key=${apiKey}`, {
  ...
  /**
   * Generates schedule optimizations for a user's upcoming goals.
   */
  ...
  private getMockResponse(msg: string): { reply: string } {
  ...
  private getMockScheduleOptimizations(goals: any[]) {
    ...
    const suggestions = sorted.map((g) => {
      ...
      if (g.priority === 'URGENT') timeBlock = '09:00 AM - 10:30 AM (Peak Focus)';
      else if (g.priority === 'HIGH') timeBlock = '10:45 AM - 12:00 PM (High Energy)';
      else if (g.priority === 'MEDIUM') timeBlock = '01:30 PM - 03:00 PM (Post-lunch block)';
      else timeBlock = '03:30 PM - 04:30 PM (Low intensity block)';
